...
class VKontakte extends \yii\authclient\clients\VKontakte
{
    ...
    protected function initUserAttributes()
    {
        ...
        if ($attributes['response'][0]['city']){
            $city = $this->api('database.getCitiesById', 'GET', [
                'city_ids' => implode(',', [
                    $attributes['response'][0]['city'],
                ]),
            ]);
            if ($city){
        ...
        $return_attributes = [
            'User' => [
                ...
                'city' => (isset($city) && array_keys(\Yii::$app->params['cities'], $city) ? array_keys(\Yii::$app->params['cities'], $city)[0] : 1),
