...
class UserController extends \yii\web\Controller
{
    ...
    public function actionSignup()
    {
        ...
        if (Yii::$app->request->post('reg_name')){
            $model->username = Yii::$app->request->post('reg_name');
        }
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($user = $model->signup()) {
                if ($user->createEmailConfirmToken() && $user->sendEmailConfirmationMail(Yii::$app->controller->module->getCustomMailView('confirmNewEmail'), 'new_email')) {
                    Yii::$app->getSession()->setFlash('success', '<div class="alert alert-success">'.Yii::t('users', 'CHECK_YOUR_EMAIL_FOR_FURTHER_INSTRUCTION').'</div>');
                    $transaction->commit();
    ...
    public function actionProfile()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            if (Yii::$app->request->post('delete-profile-photo')){
                foreach (Yii::$app->request->post('delete-profile-photo') as $key => $value) {
                    PhotosForUser::findOne($value)->delete();
                }
            }
            if ($model->photos){
                if ($photos = $model->photos($model->id)) {
                    if ((count($photos) + count($model->photoss)) > 5){
                        $model->addError('photos', 'Максимальное количество фотографий для вашего аккаунта: 5');
                    ...
                    foreach ($photos as $key => $value) {
                        $photo_model = new PhotosForUser();
                        $photo_model->user_id = $model->id;
                        $photo_model->photo_src = $value;
                        $photo_model->save();
                    }
                }
            }
